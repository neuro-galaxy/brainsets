...
def merge_probes(spikes_list, clusters_list):
    ...
    assert len(clusters_list) == len(
        spikes_list
    ), "clusters_list and spikes_list must have the same length"
    assert all(
        [isinstance(s, dict) for s in spikes_list]
    ), "spikes_list must contain only dictionaries"
    assert all(
        [isinstance(c, pd.DataFrame) for c in clusters_list]
    ...
    merged_spikes = {
        k: np.concatenate([s[k] for s in merged_spikes])
...
def list_brain_regions(neural_dict, **kwargs):
    ...
    beryl_reg = brainreg.acronym2acronym(
        neural_dict["cluster_regions"], mapping="Beryl"
    )
    regions = (
...
def get_spike_data_per_interval(
    times, clusters, interval_begs, interval_ends, interval_len, binsize, n_workers=0
):
    """
    ...
    if False and n_workers > 0:
        ...
        # Fallback to sequential processing if n_workers = 0
        intervals = list(zip(np.arange(n_intervals), interval_begs, interval_ends))
        with tqdm(total=len(intervals)) as pbar:
...
def bin_spiking_data(
    reg_clu_ids, neural_df, intervals=None, trials_df=None, n_workers=1, **kwargs
):
    """
    ...
    try:
        ...
        # ['times', 'bodyMotionEnergy']
            ...
            beh_dict = {
                ...
                "values": sess_loader.motion_energy["leftCamera"][
                    "whiskerMotionEnergy"
                ].to_numpy(),
            }
            ...
            beh_dict = {
                ...
                "values": sess_loader.motion_energy["rightCamera"][
                    "whiskerMotionEnergy"
                ].to_numpy(),
            }
        ...
        # TO DO: Different cameras have very different traces for the same behavior. Treat them as independent?
            ...
            lp_left = one.load_object(
                eid, f"leftCamera", attribute=["lightningPose", "times"]
            )
            dm1 = np.fabs(
            ...
            beh_dict = {"times": lp_left["times"], "values": dm1}
        elif target == "lightning-pose-right-pupil-diameter":
            lp_right = one.load_object(
                eid, f"rightCamera", attribute=["lightningPose", "times"]
            )
            dm1 = np.fabs(
            ...
            assert np.allclose(dm1, dm2)
            beh_dict = {"times": lp_right["times"], "values": dm1}
            ...
            beh_dict = {
                "times": dlc_left.times,
                "values": dlc.get_speed(
                    dlc_left.dlc, dlc_left.times, camera="left", feature="paw_l"
                ),
            }
            ...
            beh_dict = {
                "times": dlc_left.times,
                "values": dlc.get_speed(
                    dlc_left.dlc, dlc_left.times, camera="left", feature="paw_r"
                ),
            }
            ...
            beh_dict = {
                "times": dlc_right.times,
                "values": dlc.get_speed(
                    dlc_right.dlc, dlc_right.times, camera="right", feature="paw_l"
                ),
            }
            ...
            beh_dict = {
                "times": dlc_right.times,
                "values": dlc.get_speed(
                    dlc_right.dlc, dlc_right.times, camera="right", feature="paw_r"
                ),
            }
            ...
            beh_dict = {
                "times": dlc_left.times,
                "values": dlc.get_speed(
                    dlc_left.dlc, dlc_left.times, camera="left", feature="nose_tip"
                ),
            }
            ...
            beh_dict = {
                "times": dlc_right.times,
                "values": dlc.get_speed(
                    dlc_right.dlc, dlc_right.times, camera="right", feature="nose_tip"
                ),
            }
        else:
...
def load_anytime_behaviors(one, eid, n_workers=0):
    ...
    behave_dict = {}
    # Direct sequential loading without multiprocessing
    for beh in behaviors:
        try:
            behave_dict[beh] = load_target_behavior(one, eid, beh)
        except Exception as e:
            print(f"Error loading behavior {beh}: {e}")
            behave_dict[beh] = {"times": None, "values": None, "skip": True}
    ...
    for beh in behaviors:
        if beh == "whisker-motion-energy":
            ...
            if "skip" in target_dict.keys():
                target_dict = load_target_behavior(
                    one, eid, "right-whisker-motion-energy"
                )
        elif beh == "pupil-diameter":
            target_dict = load_target_behavior(
                one, eid, "lightning-pose-left-pupil-diameter"
            )
            if "skip" in target_dict.keys():
                target_dict = load_target_behavior(
                    one, eid, "lightning-pose-right-pupil-diameter"
                )
        else:
...
def prepare_data(one, eid, bwm_df, params, n_workers=0):
    ...
    trials_df, trials_mask = load_trials_and_mask(
        one=one, eid=eid, min_rt=None, max_rt=None, max_trial_len=None
    )
...
def align_spike_behavior(binned_spikes, binned_behaviors, trials_mask=None):
    ...
    for beh_name in beh_names:
        beh_mask = [
            1 if trial is not None else 0 for trial in binned_behaviors[beh_name]
        ]
    ...
    for beh_name in beh_names:
        aligned_binned_behaviors.update(
            {beh_name: np.delete(binned_behaviors[beh_name], del_idxs, axis=0)}
        )
        ...
        if beh_name in [
            ...
            aligned_binned_behaviors[beh_name] = (
                aligned_binned_behaviors[beh_name]
                - np.min(aligned_binned_behaviors[beh_name])
            ) / (
                np.max(aligned_binned_behaviors[beh_name])
                - np.min(aligned_binned_behaviors[beh_name])
            )
        assert len(aligned_binned_spikes) == len(
            aligned_binned_behaviors[beh_name]

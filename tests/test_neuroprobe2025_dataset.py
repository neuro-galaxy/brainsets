...
def _channel_mask_key(
    *,
    ...
    lite_bit, nano_bit = VERSION_BITS[version]
    binary_tasks_bit = MODE_TO_BINARY_TASKS_BIT[label_mode]
    return (
        f"included$lite{lite_bit}$nano{nano_bit}$binary_tasks{binary_tasks_bit}$"
        f"{h5_regime}${task}$fold{fold}${split}"
    )
...
def _write_mock_h5(
    ...
    *,
    ...
    with h5py.File(path, "w") as h5:
        ...
        fold_group = (
            splits_group.create_group(version)
            .create_group(label_mode)
            .create_group(h5_regime)
            .create_group(task)
            .create_group(f"fold{fold}")
        )

        for split in splits:
            interval_group = fold_group.create_group(f"{split}_intervals")

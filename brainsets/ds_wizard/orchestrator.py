...
class Orchestrator:
    ...
    async def _run_agent(
        ...
        log_agent_start(self._console, config["name"], config["description"], config["color"])
    ...
    def _setup_workspace(self, dataset_id: str, context: Dict[str, Any] = None) -> tuple:
    ...
    def _assemble_dataset(self, results: Dict[str, Any]) -> Optional[Dataset]:
        ...
        try:
            ...
            if not all([metadata, channel_maps, recording_info]):
                missing = [k for k, v in [("metadata", metadata), ("channel_maps", channel_maps), ("recording_info", recording_info)] if not v]
    ...
    async def run(self, dataset_id: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "error" not in channel_result:
                ...
                recording_result = await self._run_agent("recording", dataset_id, recording_ctx)
